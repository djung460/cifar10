...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        # Get the mean
        batch_mu = 1.0 / float(N*in_height*in_width) * np.sum(x,axis=(0,2,3)).reshape(1, in_depth, 1, 1)
        
        # Get the variance
        batch_var = 1/ float(N*in_height*in_width) * np.sum((x-batch_mu)**2,axis=(0,2,3)).reshape(1, in_depth, 1, 1)
        ...
        running_mean = momentum * running_mean + (1.0 - momentum) * np.squeeze(batch_mu)
        running_var = momentum * running_var + (1.0 - momentum) * np.squeeze(batch_var)
